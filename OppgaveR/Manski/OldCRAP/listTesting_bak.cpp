...
Rcpp::List redBlue2(std::vector < double > xx1,
  ...
  /* For each iteration we will need to know where we are in the isRed and weights list.
   * We use lists because we really don't need random access --- we don't jump around a lot,
   * or at all, really. The only operation we need to do on these lists is to remove the 
   * current element, run redBlue on the smaller lists, and insert the element back again. */
  ...
  for (int lineCount = 0; lineCount < nobs; lineCount++){
    ...
    std::cout << "Weight:" << getWeightMax(redBlues) << std::endl;
    if (tempWeight < maxWeight){
        ...
        for (int i = 0; i < (nobs + 1); i++){
            if (redBlues[i].posWeight >= maxWeight) {
                std::cout << lineCount << "::" << redBlues[i].posWeight << "; " << redBlues[i].value << std::endl;
 ...
 std::vector < double > xs(nobs + 1);
 std::vector < double > ys(nobs + 1);
 std::vector < double > cols(nobs + 1); 
 std::vector < double > weightz(nobs + 1); 

 for (int i = 0; i < (nobs + 1);i++){
   xs[i]      = redBlues[i].value;
   ys[i]      = -1/xx1[nobs-1] - xx2[nobs-1]/xx1[nobs-1]*xs[i];
   cols[i]    = redBlues[i].isRed;
   weightz[i] = redBlues[i].posWeight;
 }

  
 return Rcpp::List::create(Rcpp::Named("xCoord")    = xCoord,
                           Rcpp::Named("yCoord")    = yCoord,
                           Rcpp::Named("colors")    = cols,
                           Rcpp::Named("cols")      = colors,
                           Rcpp::Named("points")    = points,
                           Rcpp::Named("weights")   = weightz,
                           Rcpp::Named("xs")        = xs,
                           Rcpp::Named("ys")        = ys,     
                           Rcpp::Named("signum")    = signum(-.5),
                           Rcpp::Named("maxWeight") = maxWeight);
}
