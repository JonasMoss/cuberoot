...
##################################################
a
sdeiter <- function(data,k=10,m=50){
  n = length(data)
  data = sort(data)
  ranks = floor(c(0,seq(1,n,len=k-1),n+1))
  rank_test = 0*ranks
  counter = 0
  aug = c(0,data,1)
...
sdesub = function(data,k=10,alpha=0.5){
  n = length(data)
  q = rowMeans(replicate(1000,sdeiter(sample(data,alpha*n),k)))
  class(q) = c("sde","numeric")
  q
}
...
sdegc = function(data,k=10,K=length(data)*200*k,seed=1828){
  old = .Random.seed
  set.seed(seed)
  q = sdeiter(rgcde(K,data),k)
  set.seed(old)
  q
}

sde = function(data,k=10,method="gc",seed=1828){
  if (method=="gc") {
      sdegc(data,k,seed=seed)
    } else if (method=="iter") {
      sdeiter(data,k)
    } else if (method=="sub") {
      sdesub(data,k)
    } else {
      stop("Please provide a valid method. Allowed methods are 'gc', 'iter' and 'sub'.")
    }
...
sde_true <- function(P,Q=NULL,k,...){
  ...
  ret = constrOptim.nl(q0,sde_objective,hin=hin,Pn=P,k=k,control.outer=list(trace=FALSE),...)$par
  class(ret) = c("sde","numeric")
  ret
}
